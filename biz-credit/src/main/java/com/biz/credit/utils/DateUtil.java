...
public class DateUtil {
    ...
    /**
     * 获得指定时间加减参数后的日期(不计算则输入0)
     * 处理7D-近七天 6M-近6个月 2Y-近2年 这样的日期格式
     *
     * @param date   指定日期
     * @param period 周期
     * @return 计算后的日期
     */
    public static Date addDate(Date date, String period, int size) {
        Calendar c = Calendar.getInstance();
        c.setTime(date);
        if (period.equals("D")) {
            c.add(Calendar.DATE, -size);//加减天数
            return c.getTime();
        } else if (period.equals("M")) {
            c.add(Calendar.MONTH, -size);//加减天数
            return c.getTime();
        } else if (period.equals("Y")) {
            c.add(Calendar.YEAR, -size);//加减天数
            return c.getTime();
        }
        return null;
    }
    ...
    public static void main(String[] args) {
        try {
            FileUtils.write(new File("C:/Users/yangjinping/Desktop/htmltopdf2/a.html"), "ss", "UTF-8");
