...
@Repository
public class FixedPriceModelVarDAO {
    @Autowired
    private JdbcTemplate jdbcTemplate;
    public List<FixedPriceModelVarVO> findModelVarList(Integer modelId){
        String sql = "select model_id, api_prod_code, api_version, var_code, var_name, var_version, var_interval, var_json_path, param_name, status,date_format(create_time,'%Y-%m-%d %T') create_time from t_fixed_price_model_var where status=1 and model_id=? order by create_time desc";
        List<FixedPriceModelVarVO> list = jdbcTemplate.query(sql,(rs, i)->new FixedPriceModelVarVO(
                rs.getInt("model_id")
